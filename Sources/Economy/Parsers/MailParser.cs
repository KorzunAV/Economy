...
namespace Economy.Parsers
{
    public class MailParser
    {
        ...
        private static List<CurrencyTypeDto> _currencyTypeDtos;

        protected List<CurrencyTypeDto> CurrencyTypeDtos
        {
            get { return _currencyTypeDtos ?? (_currencyTypeDtos = CommandQueryDispatcher.ExecuteQuery<List<CurrencyTypeDto>>(new CurrencyTypeGetAllQuery()).Data); }
        }


        public MailParser(ICommandQueryDispatcher commandQueryDispatcher)
        {
        ...
        protected string RemoveAttributs(string text)
        {
            ...
            for (var index = 0; index < blocs.Length; index++)
            {
                ...
                var start = bloc.IndexOf(" ", StringComparison.Ordinal);
                if (start != -1 && bloc[0] != 'a')
                {
                    var end = bloc.IndexOf(">", StringComparison.Ordinal);
                    if (start < end)
                    {
